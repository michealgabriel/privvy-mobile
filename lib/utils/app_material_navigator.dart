...
void appNavigate(BuildContext context, Widget widget, {bool isPushReplace = false}) {
  isPushReplace 
  ? Navigator.pushReplacement(context, MaterialPageRoute(fullscreenDialog: true, builder: (context) => widget))
  : Navigator.push(context, MaterialPageRoute(fullscreenDialog: true, builder: (context) => widget));
}
...
void launchBottomSheet(BuildContext context, Widget sheetWidget, bool isScrollControlled, {bool? displayNotch = true, double? fixedHeight}) {
...
Future launchPopUp(BuildContext context, Widget popupWidget, {List<Widget>? actions, bool? dismissible}) async {
      ...
      return WillPopScope(
        ...
        child: AlertDialog(
          ...
          content: Container(
            ...
            padding: EdgeInsets.symmetric(vertical: 20, horizontal: isTablet ? 20 : 0),
 ...
 void showAppToast(BuildContext context, String toastMessage, {bool? isError}) {
    showToastWidget(
      AppToast(message: toastMessage, isError: isError,),
      context: context,
      isIgnoring: false,
      animation: StyledToastAnimation.slideFromTopFade,
      duration: isError != null && isError == true ? const Duration(seconds: 4) : const Duration(seconds: 4),
    );
  }