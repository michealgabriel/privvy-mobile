...
class PrivvyScreen extends StatefulWidget {
  ...
  const PrivvyScreen({super.key, required this.autoID, required this.imageName, required this.category});
...
class _PrivvyScreenState extends State<PrivvyScreen> {
  ...
  GenerationResponseModel generationResponse = GenerationResponseModel(message: '', colors: [], images: [], imageUrls: []);
  ...
  void initializeTimer() {
    ...
    context.read<TimerCountdownProvider>().setTimerEndCallback(onTimerEndCallback);
  ...
  callGenerateVariationsApi() async {
    try {
      generationResponse = await ApiService().callGenerateVariationsApi(widget.autoID, widget.imageName);

      if(mounted) {
        selectedColorIndex = 0;
        Provider.of<PrivvyGenerationProvider>(context, listen: false).setIsCallingGenerateAPI(false);
  ...
  Future<bool> _onLoadingWillPop() async {
    ...
    await launchPopUp(
      ...
      const Column(
        children: [
          Column(
            ...
            children: [
              Text('Exit screen and move task to background?', textAlign: TextAlign.start, style: AppThemeConstants.APP_BODY_TEXT_REGULAR,),
              // SizedBox(height: 10,),
            ],
          ),
        ],
      ...
      actions: [
        ...
        TextButton(
          child: Text('Exit', style: isTablet ? AppThemeConstants.APP_BODY_TEXT_LARGE : AppThemeConstants.APP_BODY_TEXT_MEDIUM,),
          onPressed: () {
            Navigator.of(context).pop();
  ...
  Future<bool> _onWillPop() async {
    ...
    await launchPopUp(
        ...
        children: [
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Collection Name', textAlign: TextAlign.start, style: AppThemeConstants.APP_BODY_TEXT_REGULAR,),
      ...
      actions: [
        ...
        TextButton(
          child: Text('Save & Exit', style: isTablet ? AppThemeConstants.APP_BODY_TEXT_LARGE : AppThemeConstants.APP_BODY_TEXT_MEDIUM,),
          onPressed: () async {
            ...
            if(_collectionNameController.text.isNotEmpty) {
              ...
              Map<String, dynamic> updateData = {"name": _collectionNameController.text, "category": widget.category};
              await DatabaseService.updateRecord("${AppConstants.usersMetadataDBCollectionName}/$authenticatedUserID/collections/${widget.autoID}", updateData);
            }
  ...
  Color hexTransform(int indexPosition) {
    return Color(int.parse(generationResponse.colors[indexPosition].replaceAll('#', '0xFF')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final minutes = strDigits(context.select<TimerCountdownProvider, int>((provider) => provider.myDuration.inMinutes.remainder(60)));
    final seconds = strDigits(context.select<TimerCountdownProvider, int>((provider) => provider.myDuration.inSeconds.remainder(60)));

    return WillPopScope(
      onWillPop: () {
        if(context.read<PrivvyGenerationProvider>().isCallingGenerateAPI) return _onLoadingWillPop();
        return _onWillPop();
      },
      ...
      child: Scaffold(
            ...
            child: Row(
              ...
              children: [
                AnimatedContainer(
                  duration: const Duration(milliseconds: 500),
                  color: _walkBoxColor, 
                  child: Lottie.asset('assets/lotties/walk.json', animate: true)
                ),
                showTimer ? Column( 
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text('$minutes : $seconds', style: AppThemeConstants.APP_HEADING_TEXT_2,),
                    const Text("Estimated Time", style: AppThemeConstants.APP_BODY_TEXT_SMALL,),
                  ],
                )
                :
                const Flexible(child: Text("🤔 Taking longer than expected...", textAlign: TextAlign.center, overflow: TextOverflow.clip, style: AppThemeConstants.APP_BODY_TEXT_LARGE,)),
              ],
            )
          ), 
        )
        // ! --------- MAIN CONTENT WIDGET
        : Container(
          padding: EdgeInsets.all(Device.screenType == ScreenType.tablet ? AppThemeConstants().APP_TABLET_BASE_CONTENT_PADDING : 0),
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [
                selectedColorIndex == null ? AppThemeConstants.APP_BG_DARK : hexTransform(selectedColorIndex!),
          ...
          child: Flex(
            ...
            children: [
              Expanded(
                child: Center(
                  child: CircleAvatar(
                    radius: Device.screenType == ScreenType.tablet ? tabletPrivvyImageRadius : privvyImageRadius,
                    backgroundColor: AppThemeConstants.APP_CARD_BG_DARK,
                    backgroundImage: NetworkImage(generationResponse.imageUrls[selectedColorIndex ?? 0]),
                  ),
                ),
              ),
              Container(
                height: Device.screenType == ScreenType.tablet ? tabletContainerPreviewSize : containerPreviewSize,
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(shape: BoxShape.circle, color: selectedColorIndex == index ? AppThemeConstants.APP_PRIMARY_COLOR : Colors.transparent),
                        padding: const EdgeInsets.all(6),
                        margin: index == 0 ? const EdgeInsets.only(left: 30) : index == generationResponse.colors.length ? const EdgeInsets.only(right: 30) : null,
                        child: Container(
                          width: Device.screenType == ScreenType.tablet ? tabletContainerPreviewSize : containerPreviewSize,
                          height: Device.screenType == ScreenType.tablet ? tabletContainerPreviewSize : containerPreviewSize,
                          decoration: BoxDecoration(
                            shape: BoxShape.circle, 
                            image: DecorationImage(image: NetworkImage(generationResponse.imageUrls[index],), fit: BoxFit.cover,),
