...
class _CollectionsState extends State<Collections> {
  ...
  fetchCollections() async {
    ...
    try {
      ...
      dynamic userCollectionsResponse = await DatabaseService.readTargetMultiRecord(
          "${AppConstants.usersMetadataDBCollectionName}/$authenticatedUserID");

      // save and convert collections data to List
      collectionsList =
          (userCollectionsResponse as Map<String, dynamic>)["collections"]
              .values
              .toList();

      // save collections key id's
      (userCollectionsResponse["collections"] as Map)
          .forEach((key, _) => collectionsKeyId.add(key.toString()));

      // error response
      if (userCollectionsResponse.toString() == AppConstants.serverException ||
          userCollectionsResponse.toString() ==
              AppConstants.noDatabaseResults) {
        if (mounted)
          showAppToast(context, userCollectionsResponse.toString(),
              isError: true);
      }
      ...
      if (mounted)
        showAppToast(context, "Unable to fetch collections", isError: true);
    }
  ...
  Future<bool> handleConfirmEdit(
      int index, String collectionId, String collectionName) async {
    bool result = false;
    bool isTablet = Device.screenType == ScreenType.tablet;
            ...
            onPressed: () async {
              ...
              if (_collectionNameController.text.isNotEmpty) {
                ...
                await DatabaseService.updateRecord(
                    "${AppConstants.usersMetadataDBCollectionName}/$authenticatedUserID/collections/$collectionId",
                    updateData);
  ...
  deleteData(String collectionId) async {
    ...
    bool storageDeleteFlag =
        await ApiService().callDeleteVariationsApi(collectionId);

    if (storageDeleteFlag) {
      deleteResult = await DatabaseService.deleteRecord(
          "${AppConstants.usersMetadataDBCollectionName}/$authenticatedUserID/collections",
          collectionId);
    }
                          ...
                          return Dismissible(
                            ...
                            confirmDismiss: (direction) async {
                              if (direction == DismissDirection.endToStart)
                                return await handleConfirmDelete(
                                    collectionsKeyId[index]);
                              if (direction == DismissDirection.startToEnd)
                                return await handleConfirmEdit(
                                    index,
                                    collectionsKeyId[index],
                                    collectionsList[index]["name"].toString());
                              return false;
                            },
                            ...
                            child: GestureDetector(
                              onTap: () => appNavigate(
                                  context,
                                  SingleCollection(
                                      collectionName: collectionsList[index]
                                              ["name"]
                                          .toString(),
                                      images: collectionsList[index]
                                          ["images"])),
                              child: Container(
                                  ...
                                  children: [
                                    ClipRRect(
                                      ...
                                      child: CachedNetworkImage(
                                        imageUrl: collectionsList[index]
                                            ["images"][0],
                                        width: 100,
                                        height: 100,
                                        fit: BoxFit.cover,
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          children: [
                                            ...
                                            Text(
                                              "${collectionsList[index]["images"].length} (items)",
                                              style: AppThemeConstants
                                                  .APP_BODY_TEXT_REGULAR,
                                            ),
                                          ],
                                        ),
                                      ],
