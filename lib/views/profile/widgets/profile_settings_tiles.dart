...
class _ProfileSettingsTilesState extends State<ProfileSettingsTiles> {
  ...
  void initState() {
    ...
    (() async {
      chillExperienceStatus = await AppSPHandlers()
          ...
          true;
      volumeSliderValue = await AppSPHandlers()
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        buildCard(Icons.person_pin, 'Change Avatar',
            tapAction: () => handleChangeAvatar()),
        buildCard(Icons.how_to_reg_sharp, 'Change Nickname',
            tapAction: () => handleChangeName()),
        buildCard(
          Iconsax.headphone5, 'Chill Experience',
        ...
        buildCard(Icons.edit_document, 'App Info',
            tapAction: () => launchPopUp(
                context,
                Column(
                  children: [
                    Text(
                      "Privvy is a free app to generate color variations that compliments each other. The application makes api calls to an OpenCV Python endpoint that does the image generations, no fancy GAN models of some sort for now.",
                    ...
                    Text(
                      "Hence, a few glitches in certain colors could be discovered for some product images.",
                      style: Device.screenType == ScreenType.tablet
                          ? AppThemeConstants.APP_BODY_TEXT_LARGE
                          : AppThemeConstants.APP_BODY_TEXT_MEDIUM,
                    ),
                    ...
                    Text(
                      "You're a dev 🧑‍💻 ?, you can contribute to the generation source code on github 😺",
                      style: Device.screenType == ScreenType.tablet
                          ? AppThemeConstants.APP_BODY_TEXT_LARGE
                          : AppThemeConstants.APP_BODY_TEXT_MEDIUM,
                    ),
        ...
        buildCard(Icons.cloud, 'App Updates',
            tapAction: () async => await launchUrl(
                Uri.parse('https://github.com/michealgabriel'))),
        buildCard(Icons.cancel_outlined, 'Delete Account',
            tapAction: () => handleDeleteAccount(), isRedGradient: true),
        const SizedBox(
          height: 100,
        ),
      ],
  ...
  Widget buildCard(IconData radiusIcon, String tileText,
      {Function? tapAction, Widget? trailWidget, bool isRedGradient = false}) {
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
            gradient: isRedGradient
                ? AppThemeConstants.APP_CARD_GRADIENT_RED
                : AppThemeConstants.APP_CARD_GRADIENT,
