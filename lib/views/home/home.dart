...
class _HomeState extends State<Home> {
  final EdgeInsetsGeometry cardPaddings =
  ...
  void initState() {
    ...
    (() async {
      ...
      chillExperienceStatus = await AppSPHandlers()
              .getBoolSP(AppSPHandlers.CHILL_EXPERIENCE_STATUS) ??
          true;
      volumeStatus = await AppSPHandlers()
              .getDoubleSP(AppSPHandlers.VOLUME_SLIDER_VALUE) ??
          0.1;

      if (mounted) {
        Provider.of<MusicProvider>(context, listen: false)
            .initializeMusics(initVolume: volumeStatus);
        if (chillExperienceStatus)
                ...
                Row(
                  children: [
                    ClipRRect(
                        ...
                        child: Image.asset(
                          "assets/thumbnails/${playlistMetaData[player.currentIndex!][0]}",
                          width: 85,
                          height: 85,
                        )),
                    const SizedBox(
                    ...
                    Wrap(
                      ...
                      children: [
                        Text(
                          playlistMetaData[player.currentIndex!][1],
                          style: AppThemeConstants.APP_BODY_TEXT_LARGE,
                        ),
                        Text(
                          playlistMetaData[player.currentIndex!][2],
  ...
  Future<bool> _onWillPop() async {
    ...
    await launchPopUp(
        context,
        Text(
          'You sure you wanna exit privvy?',
          textAlign: TextAlign.start,
          style: isTablet
              ? AppThemeConstants.APP_BODY_TEXT_LARGE
              : AppThemeConstants.APP_BODY_TEXT_REGULAR,
        ),
        actions: [
          TextButton(
            child: Text(
    ...
    if (image != null) {
      ...
      if (await imageFile.length() > maxSizeBytes) {
        if (mounted) {
          ...
          showAppToast(
              context, "Image size exceeds 1MB. Choose a smaller image",
              isError: true);
        }
      ...
      if (resultMap["collections"] != null &&
          resultMap["collections"].length >
              (AppConstants.maxCollectionsCount - 1)) {
        if (mounted) {
          Navigator.pop(context); // close sheet
          showAppToast(context,
              "Cannot have more than ${AppConstants.maxCollectionsCount} collections. Delete some",
              isError: true);
        }
      ...
      if (response != AppConstants.serverException) {
        ...
        await DatabaseService.updateRecord(
            "${AppConstants.usersMetadataDBCollectionName}/$authenticatedUserID/collections/${responseStringSplit[0]}",
            updateData);
        ...
        if (mounted) {
          ...
          context
              .read<PrivvyGenerationProvider>()
              .setIsCallingGenerateAPI(true);
          appNavigate(
              context,
              PrivvyScreen(
                autoID: responseStringSplit[0],
                imageName: responseStringSplit[1],
                category: category,
              ));
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> privvyCardItems = [
      PrivvyItemStack(
        ...
        imagePositionedLeftValue:
            Device.screenType == ScreenType.tablet ? -15 : -35,
      ...
      PrivvyItemStack(
        ...
        imagePositionedLeftValue:
            Device.screenType == ScreenType.tablet ? 60 : 40,
      ...
      PrivvyItemStack(
        ...
        imagePositionedTopValue: -110,
        imagePositionedLeftValue:
            Device.screenType == ScreenType.tablet ? 40 : 20,
        imageRotatedAngle: -0.01,
        cardAnimationDelay: 4,
    ...
    return Scaffold(
      ...
      body: WillPopScope(
        ...
        child: SingleChildScrollView(
          padding:
              EdgeInsets.all(AppThemeConstants().APP_BASE_CONTENT_PADDING - 5),
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(
                      ...
                      color: Colors.white,
                      size: Device.screenType == ScreenType.tablet ? 40 : 30,
                    ),
                      ...
                      children: [
                        ...
                        GridView.count(
                            ...
                            children: List.generate(4, (index) {
                              return Container(child: privvyCardItems[index]);
                            })),
                      ],
                    )
                  : Column(
